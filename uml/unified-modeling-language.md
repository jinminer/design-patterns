...
* `UML`箭头方向：从子类指向父类
  * 误区：可能会认为子类式以父类为基础的，从父类引申而来，所以箭头应该从父类指向子类
...
* 空心三角箭头：继承或实现
  * 实线-继承：is a 关系，扩展的目的，不虚，很结实，有具体的内容，有实现细节
...
>   * `~` ---> `default` 包权限
> * 类属性后边的`:` 表示属性的类型
> * 类属性、行为下方的下划线表示`static` 静态的属性和方法
>   * 如：<u>`+sex:char`</u> 和 <u>`+playGames()`</u>
> * 方法名斜体：抽象方法，如 *`+study()`*  
> * 方法后面`:` 再加一个类型：方法的返回类型
> * 方法`()`中的内容为参数
...
![8-class-diagram](<https://raw.githubusercontent.com/jinminer/docs/master/design-patterns/uml/8-class-diagram-multiple.png>)
...
> * 关联关系/依赖关系
>   * 依赖关系：虚线箭头
>     * 依赖对象需要依赖被依赖对象完成某种行为或方法
>     * 如：动物（依赖对象）的新陈代谢（行为、方法）需要依赖水（被依赖对象）来完成
>     * 被依赖对象一般作为依赖对象的方法（行为）的参数
>     * 依赖关系一般体现在方法上，被依赖对象只是被依赖对象作为临时的变量引用一下
>   * 关联关系：实现箭头
>     * 关联对象和被关联对象之间关系稳定
>     * 如：企鹅（关联对象）和气候（被关联对象）之间是强关系，企鹅离开南极很难存活
>     * 关联关系一般体现在属性上，被关联对象一般存作为关联对象的属性而存在
> * 组合关系/聚合关系（整体和局部的关系）
>   * 组合关系：实心菱形
>     * 组合因子是被组合对象的必备条件，两者处于相同的生命周期中
>     * 如：翅膀（组合因子）是鸟对象（被组合对象）的完全必要条件
>     * 组合对象和组合因子之间是强关系，二者密不可分，不可相互独立
>   * 聚合关系：空心菱形
>     * 聚合对象由多个聚合因子聚合而成，是`has a`的关系
>     * 如：大雁群（聚合对象）由若干个大雁（聚合因子）聚集汇合而成
>     * 聚合对象和聚合因子之间是弱关系，两者拥有独立的生命周期
> * 接口的表示方法
>   * `<<interface>>`
>   * 棒棒糖表示法
