...
### <a name="summarize-head" href="#summarize">总结</a>
...
### <a name="mediator" href="#mediator-head">中介者模式</a>

* 定义

  * 定义一个封装一组对象如何交互的对象
  * 通过使对象明确地相互引用来促进松散耦合，并允许独立改变它们的交互
...
* 适用场景
  ...
  * 交互的公共行为，如果需要改变行为则可以增加新的中介者类
    * 对于某些场景，可以通过扩展或新增中介者类，来达到行为扩展的目的
    * 例如微信群场景，如果我们想为某一群人传达某个指令，可以创建一个微信群，这样在发送群消息后，群里的每一个人都可以接收到；如果不适用群的中介关系，我们就必须把消息向每一个人都通知一遍

* 优点

  * 将一对多转化成了一对一、降低程序复杂度
    * 如微信群场景，把原先的一个人向多个人分别发消息的行为，转化为了一个人直接向一个微信群发送消息
  * 类之间解耦
    * 类和类之间通过中介类进行交互，降低了直接关联的耦合关系，如微信群成员在发群消息时，并不关心群里增员和减员
...
* 相关设计模式
  ...
  * 中介者模式和观察者模式
    * 两者有时会结合使用，通过观察者模式来实现中介者模式中角色间的通讯场景

* 代码示例

  * 场景：学生在学习交流群进行交流

    ![code-1](https://raw.githubusercontent.com/jinminer/docs/master/design-patterns/design-pattern-best/mediator/code-1.png)

* 源码实践
  * `java.util.Timer` ---> 定时任务计时器：中介者，用来对不同的定时任务进行调度
  * `java.util.TimerTask` ---> 定时任务：不同用户
...
## <a name="summarize" href="#summarize-head">总结</a>
...
* 单例和工厂模式
  * 工厂类一般就是被设计为单例
    * 如 `spring#ApplicationContext` 既是工厂又是单例
...
* 策略模式和委派模式
  ...
  * 策略模式是委派模式内部的一种实现形式，策略模式关注的是结果是否能互相替代
    * 如支付方式：`alipay` 、`wechat-pay` 它们的任务都是相同，是平台的支付通道，为用户提供支付能力，而且它们之间是可以互相替换的
  * 委派模式更关注分发和调度的过程
    * 有时会采用 `if...else...` 条件分支来分发职能，而且如果分发结果比较统一规整和类似的换，在委派模式内部可以使用策略模式来替代 `if...else...` 分支语句

* 模板方法模式和工厂方法模式

  * 工厂方法是模板方法的一种特殊实现，工厂方法模式是实现一个步骤或一部分算法逻辑的模板方法模式，即工厂方法模式是模板方法模式的子集
  * 模板方法模式中的算法逻辑(方法调用顺序)是固定的，而工厂方法模式中的算法逻辑(方法调用顺序)不是一成不变的

* 模板方法和策略模式

  * 模板方法和策略模式都有封装算法
  * 策略模式是使不同算法可以相互替换，且不影响客户端应用层的使用
  * 模板方法模式使针对定义一个算法的流程，将一些有细微差异的部分交给子类实现；而策略模式中算法的实现过程是封闭的，只需要去调用即可。
  * 模板方法模式不能改变算法流程，策略模式可以改变算法流程且可相互替换，策略模式通常用来代替 `if...else...` 等条件分支语句

* 装饰器模式和静态代理模式
  * 装饰者模式关注点在于给对象动态扩展、添加方法，而代理模式更加注重控制对象的访问
  * 代理模式通常会在代理类中创建被代理对象的实例，而装饰者模式通常把被装饰者作为构造参数
  * 装饰者模式中的被装饰者是由调用者(客户端)传入的，如构造注入 `set` 方法注入等，即你要装饰谁，就把谁给我，我再返回给你一个你期望的目标对象，目标是确定的；代理模式也需要调用者显示的指定一个对象，但是它的目的在于对被代理对象的行为增强和隐私保护
* 装饰者模式和适配器模式
  * 装饰者模式和适配器模式都属于包装器模式 `wrapper` 
  * 装饰者模式可以实现被装饰者与相同的接口或者继承被装饰者作为它的子类，而适配器和被装饰者可以实现不同的接口
  * 对于装饰器模式而言，无论怎么装饰，最终结果都是来自于同一个父类，类之间有继承关系 `is a`，它主要是用来处理同一类具有细微差别的事物；而适配模式则不竟然，它主要的目的是解决不同类之间进行调用时的兼容问题，是 `has a` 的关系；即装饰器模式中，类的耦合性更高，适配器模式对于被适配者的依赖相对较弱。

* 适配器和静态代理模式
  * 适配器可以结合静态代理来实现，保存被适配对象的引用，但不是唯一的实现。
* 适配器模式和策略模式
